...
class StatisticsQueries:
    def __init__(self, db: SQLAlchemy, model: Model) -> "StatisticsQueries":
        self.db = db
        self.model = model

    def _add_date_filter_to_query(self,query: BaseQuery,start_date: datetime.datetime,end_date: datetime.datetime,) -> BaseQuery:
        
        return query.filter(self.model.date.between(start_date, end_date))

    def get_number_of_unique_visitors(self, start_date: datetime.datetime, end_date: datetime.datetime) -> int:
    ...
    def get_routes_data(self, start_date: datetime.datetime, end_date: datetime.datetime) -> List:
    ...
    def get_requests_for_path(self, path: str, start_date: datetime.datetime, end_date: datetime.datetime) -> List:
        
        query = (
            self.db.session.query(self.model)
            .filter(self.model.path.startswith(path))
    ...
    def get_refferers_for_path(self,start_date: datetime.datetime,end_date: datetime.datetime,path: str = None,):
        
        query = self.db.session.query(self.model.date, self.model.referrer)
        query = self._add_date_filter_to_query(query, start_date, end_date)
        ...
        return [x[1] for x in requests]


    def get_user_chart_data(self,start_date: datetime.datetime,end_date: datetime.datetime,path: str = None,) -> Tuple[List[dict], List[dict]]:
